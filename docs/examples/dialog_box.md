...
Several settings answered in one dialog. Every field opens prefilled with its
default, so confirming as is keeps the defaults; a value out of range is
...
def test_form_with_pass_fail():
    dbx = DialogBox(
        ...
        widget=FormWidget([NumberField(name="marks", label="Marks counted", minimum=0)]),
        pass_fail=True,
    )
    response = run_dialog_box(dbx)
